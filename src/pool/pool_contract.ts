...
export class PoolContract extends Contract {
  ...
  static spec: ContractSpec = new ContractSpec([
    ...
    'AAAAAAAAAAAAAAAKaW5pdGlhbGl6ZQAAAAAABwAAAAAAAAAFYWRtaW4AAAAAAAATAAAAAAAAAARuYW1lAAAAEQAAAAAAAAAGb3JhY2xlAAAAAAATAAAAAAAAAApic3RvcF9yYXRlAAAAAAAEAAAAAAAAAAxtYXhfcG9zdGlvbnMAAAAEAAAAAAAAAAtiYWNrc3RvcF9pZAAAAAATAAAAAAAAAAdibG5kX2lkAAAAABMAAAAA',
  ...
  static readonly parsers = {
    ...
    newInterestAuction: (result: string): AuctionData =>
      PoolContract.spec.funcResToNative('new_interest_auction', result),
  };

  initialize(contractArgs: PoolInitializeArgs): string {
    return this.call(
      'initialize',
      ...PoolContract.spec.funcArgsToScVals('initialize', contractArgs)
    ).toXDR('base64');
  }
  ...
  queueSetReserve(contractArgs: SetReserveArgs): string {
    return this.call(
      'queue_set_reserve',
      ...PoolContract.spec.funcArgsToScVals('queue_set_reserve', contractArgs)
    ).toXDR('base64');
  }
  ...
  newLiquidationAuction(contractArgs: NewLiqudiationAuctionArgs): string {
    return this.call(
      ...
      ...PoolContract.spec.funcArgsToScVals('new_liquidation_auction', contractArgs)
    ).toXDR('base64');
  }
  ...
  newBadDebtAuction(): string {
    return this.call(
      ...
      ...PoolContract.spec.funcArgsToScVals('new_bad_debt_auction', {})
    ).toXDR('base64');
  }
  ...
  newInterestAuction(assets: Array<Address | string>): string {
    return this.call(
      ...
      ...PoolContract.spec.funcArgsToScVals('new_interest_auction', { assets })
    ).toXDR('base64');
  }
}
